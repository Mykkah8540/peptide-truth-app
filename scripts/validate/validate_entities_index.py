...
def main():
    ...
    # Optional: topics (extension) generated by build_entities_index.py
    topics = idx.get("topics", [])
    if topics is None:
        topics = []
    if not isinstance(topics, list):
        fail("topics must be a list when present")
    ...
    if isinstance(counts, dict):
        ...
        if counts.get("total") != (len(peptides) + len(blends) + len(topics)):
            fail("counts.total does not match peptides+blends(+topics)")

    # Topics validation (optional)
    topic_slugs = set()
    for t in topics:
        if not isinstance(t, dict):
            fail("entities_v1.json: each topic must be an object")
        if t.get("kind") != "topic":
            fail("entities_v1.json: topic.kind must be \"topic\"")
        slug = t.get("slug")
        tid = t.get("topic_id")
        dn = t.get("display_name")
        sp = t.get("source_path")
        for k, v in [("slug", slug), ("topic_id", tid), ("display_name", dn), ("source_path", sp)]:
            if not isinstance(v, str) or not v.strip():
                fail(f"entities_v1.json: topic missing/invalid {k}")
        if slug in topic_slugs:
            fail(f"entities_v1.json: duplicate topic slug: {slug}")
